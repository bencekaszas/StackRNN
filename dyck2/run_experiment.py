...
from visualize import (evaluate_and_visualize_dyck, plot_dyck_stack_viz, 
                            plot_dyck_state_trajectory, plot_dyck_ood_generalization, plot_sequence_entropy)

OUTPUT_DIR = "../results/dyck2/baseline"
...
def train_step(state, batch):
    inputs, targets, mask, target_acts, act_masks = batch
    def loss_fn(params):
        logits, action_probs, _ = state.apply_fn({'params': params}, x=inputs)
        loss_seq = masked_loss(logits, targets, mask)
        
        target_act_one_hot = jax.nn.one_hot(target_acts, NUM_MEM_ACTIONS)
        loss_act = -jnp.sum(target_act_one_hot * jnp.log(action_probs + 1e-9), axis=-1)
        loss_act = (loss_act * act_masks).sum() / jnp.maximum(act_masks.sum(), 1e-9)
        
        loss = loss_seq + loss_act
...
def evaluate_dyck(state, prompt, max_len=100, hard_actions=True):
    ...
    # 1. Encoder Phase
    _, _, carry = state.apply_fn({'params': state.params}, x=prompt, hard_actions=hard_actions)
...
def run_ood_evaluation(state, test_lengths, hard_actions, n_samples=50):
    ...
    for L in test_lengths:
        ...
        for _ in range(n_samples):
            prefix, expected_suffix, _, _ = generate_dyck_string(STACK_DEPTH-10, L)
...
if __name__ == "__main__":
    ...
    for L in [20, 100]:
        prefix, _, _, _ = generate_dyck_string(STACK_DEPTH-10, L)
        ...
        plot_sequence_entropy(stack_hist, len(prefix), os.path.join(OUTPUT_DIR, f"sequence_entropy_top_L{L}.pdf"), os.path.join(OUTPUT_DIR, f"sequence_entropy_total_L{L}.pdf"))
