...
def plot_dyck_stack_viz(full_sequence, stack_history, action_history, file_path):
    ...
    stack_probs = (stack_history[:num_actions, :, DYCK_STACK_OPEN1] + stack_history[:num_actions, :, DYCK_STACK_OPEN2]).T
    ...
    for t in full_sequence[:num_actions]:
        if t == DYCK_OPEN1: token_labels.append('(')
        elif t == DYCK_CLOSE1: token_labels.append(')')
        elif t == DYCK_OPEN2: token_labels.append('[')
        elif t == DYCK_CLOSE2: token_labels.append(']')
...
def plot_sequence_entropy(stack_history, prompt_len, file_path_top, file_path_total):
    ...
    for t in range(len(stack_history)):
        ...
        p_safe_all = np.where(p_all > 1e-9, p_all, 1.0)
