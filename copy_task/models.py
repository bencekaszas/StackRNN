...
class StackRNNCell(nn.Module):
    ...
    def __call__(self, carry, x_emb):
        ...
        logits_state = nn.Dense(self.num_states)(flat_input)
...
class StackRNN(nn.Module):
    ...
    def __call__(self, x, hard_actions=False):
        ...
        init_state = jnp.zeros((batch_size, self.num_states), dtype=jnp.float32)
        carry = (init_stack, init_state)
        
        scan_layer = nn.scan(self.cell_cls, variable_broadcast="params", 
                             split_rngs={"params": False}, in_axes=1, out_axes=1)
        
        final_carry, (logits_buf, _) = scan_layer(hard_actions=hard_actions, num_states=self.num_states)(carry, x_emb)
